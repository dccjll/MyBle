...
import com.paiai.mble.util.FileUtils;
import com.paiai.mble.util.LogUtils;
import com.paiai.mble.util.MacAddressUtils;
...
public class DeviceFirewareUpdate {
    ...
    private final BLEScan.OnBLEScanListener onBLEScanListener = new BLEScan.OnBLEScanListener() {
        ...
        public void onFoundDevice(BluetoothDevice bluetoothDevice, int rssi, byte[] scanRecord) {
            LogUtils.i(TAG, "onScanResult,扫描到设备,mac=" + bluetoothDevice.getAddress() + ",name=" + bluetoothDevice.getName());
            if (bluetoothDevice.getAddress().equalsIgnoreCase(newTargetMacAddress)) {//找到短地址
                bleManage.stopScan();
                if (startDFU) {
                    LogUtils.e(TAG, "找到新地址，但是更新服务已在运行，不再重新启动更新服务");
                    return;
                }
                LogUtils.i(TAG, "找到新地址，表示设备已经进入了更新模式，直接启动dfu更新");
//                bleManage.stopScan();
                ...
                bleManage.stopScan();
                if (sendDFUCmd) {
                    LogUtils.i(TAG, "找到原始地址，但是已发送进入固件更新模式指令，不再重复发送");
                    return;
                }
                LogUtils.i(TAG, "找到原始地址，发送进入固件更新模式指令");
//                bleManage.stopScan();
    ...
    private final DfuProgressListener dfuProgressListener = new DfuProgressListener() {
        ...
        public void onDeviceConnecting(String deviceAddress) {
            LogUtils.e(TAG, "dfu正在连接...");
        }

        @Override
        public void onDeviceConnected(String deviceAddress) {
            LogUtils.e(TAG, "dfu已连接");
        }

        @Override
        public void onDfuProcessStarting(String deviceAddress) {
            LogUtils.e(TAG, "dfu正在在启动...");
            if (onSimpleDfuProgressListener != null) {
        ...
        public void onDfuProcessStarted(String deviceAddress) {
            LogUtils.e(TAG, "dfu已启动");
        }

        @Override
        public void onEnablingDfuMode(String deviceAddress) {
            LogUtils.e(TAG, "正在启用dfu模式...");
        }
        ...
        public void onFirmwareValidating(String deviceAddress) {
            LogUtils.e(TAG, "dfu正在验证固件...");
        }

        @Override
        public void onDeviceDisconnecting(String deviceAddress) {
            LogUtils.e(TAG, "dfu正在断开连接...");
        }
        ...
        public void onDfuCompleted(String deviceAddress) {
            updateFinished = true;
            LogUtils.i(TAG, "设备上更新固件成功");
            if (new File(firewareAddress).exists()) {
                FileUtils.deleteFile(firewareAddress);
            }
            DfuServiceListenerHelper.unregisterProgressListener(Utils.getApp(), dfuProgressListener);
            if (onSimpleDfuProgressListener != null) {
        ...
        public void onError(String deviceAddress, int error, int errorType, String message) {
            if (The_Num_To_Attempt ++ < The_Max_Num_To_Attempt) {
                LogUtils.i(TAG, "第" + The_Num_To_Attempt + "次固件更新失败," + message + ",尝试第" + (The_Num_To_Attempt + 1) + "次更新...");
                startDFU = false;
                sendDFUCmd = false;
                progress = 0;
            ...
            LogUtils.i(TAG, "第" + The_Num_To_Attempt + "次固件更新失败," + message + ",已尝试最大重试次数" + The_Max_Num_To_Attempt + ",不再尝试，固件更新失败");
            DfuServiceListenerHelper.unregisterProgressListener(Utils.getApp(), dfuProgressListener);
    ...
    private void checkParams() {
        if (!MacAddressUtils.checkAddress(deviceMac)) {
            handleError(deviceMac, -10003);
            return;
        }
        if (TextUtils.isEmpty(firewareAddress)) {
            LogUtils.e(TAG, "TextUtils.isEmpty(firewareAddress) false");
            handleError(deviceMac, -10045);
            return;
        }
        if (!caculateTargetAddress()) {
            LogUtils.e(TAG, "进入固件更新的mac地址转换失败");
...
//        bleManage.setTargetDeviceAddress(deviceMac);
        ...
        LogUtils.i(TAG, "固件地址指向一个本地地址");
        if (!new File(firewareAddress).exists()) {
            LogUtils.e(TAG, "固件地址指向一个本地地址，但是该地址不存在任何文件");
            handleError(deviceMac, -10047);
            return;
        }
        LogUtils.i(TAG, "固件地址指向一个本地地址，文件存在");
...
//                LogUtils.i(TAG, "文件下载成功，路径为：" + firewareAddress);
//                if (!new File(firewareAddress).exists()) {
//                    LogUtils.e(TAG, "固件地址指向一个内部地址，但是该地址不存在任何文件");
//                    handleError(deviceMac, -10047);
//                    return;
//                }
//                LogUtils.i(TAG, "固件地址指向一个内部地址，文件存在");
//                final String extension = "(?i)ZIP"; // (?i) = case insensitive
//                final boolean statusOk = MimeTypeMap.getFileExtensionFromUrl(firewareAddress).matches(extension);
//                if (!statusOk) {
//                    LogUtils.e(TAG, "文件格式检测失败");
...
//                LogUtils.e(TAG, BLEMsgCode.parseMessageCode(integer));
...
//        });
    ...
    private boolean caculateTargetAddress() {
        ...
        LogUtils.i(TAG, "进入DFU前的设备地址=" + targetmacaddress + "\n进入DFU后的设备地址=" + newTargetMacAddress);
        return true;
    }
    ...
    private void initDFU(){
        ...
        sendDFUCmd = false;
        progress = 0;
        DfuServiceListenerHelper.registerProgressListener(Utils.getApp(), dfuProgressListener);
        if(BLEManage.checkConnectStatus(deviceMac)){
            LogUtils.i(TAG, "设备已连接，请发送命令让设备进入固件更新模式");
            if (onSimpleDfuProgressListener != null) {
    ...
    private void startScan() {
        try {
            bleManage.startScan();
            LogUtils.i(TAG, "已开始扫描");
        } catch (Exception e) {
            e.printStackTrace();
            bleManage.stopScan();
            LogUtils.e(TAG, "开始扫描设备发生异常");
    ...
    private void postDataToBLEDevice(BluetoothDevice device) {
        ...
        starter.start(Utils.getApp(), DfuService.class);
        LogUtils.i(TAG, "starter started!");
    ...
    private  void handleError(final String deviceMac, final int errorCode) {
        LogUtils.e(TAG, BLEMsgCode.parseMessageCode(errorCode));
//        if (new File(firewareAddress).exists()) {
//            FileUtils.deleteFile(firewareAddress);
//        }
