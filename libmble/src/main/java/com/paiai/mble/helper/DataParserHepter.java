...
public class DataParserHepter {

    /**
     * 反射封装网络数据包
     * 目前只能封装一个json数组的字符串，json数组中的每个元素以T对象来封装，最后以列表返回
     * 如果jsonArrayString是空的，则返回一个空元素的列表；如果T对象的字节码为空，则返回一个将jsonArrayString作为唯一元素的列表
     */
            ...
            for (int index=0;index<jsonArray.length();index++) {
                ...
                for (Map.Entry<String, Class> entry : fieldList.entrySet()) {
                    ...
                    if (jsonObject.toString().contains("\"" + fieldName + "\"")) {
                        ...
                        String methodName = "set" + fieldName.replaceFirst(firstChar, firstChar.toUpperCase(Locale.getDefault()));
                        ...
                        try {
                            ...
                            if(fieldValue != null){
                                if ("true".equalsIgnoreCase(fieldValue) || "false".equalsIgnoreCase(fieldValue)) {
                                    method.invoke(object, Boolean.parseBoolean(fieldValue));
                                } else {
    ...
    public static String loadObjectJsonString(Object object) {
        ...
        for (Map.Entry<String, Class> entry : fieldList.entrySet()) {
            String fieldName = entry.getKey();
            String firstChar = fieldName.substring(0, 1);
            String methodName = "get" + fieldName.replaceFirst(firstChar, firstChar.toUpperCase(Locale.getDefault()));
            ...
            try {
                ...
                methodName = "is" + fieldName.replaceFirst(firstChar, firstChar.toUpperCase(Locale.getDefault()));
                try {
                    methodParamType = getMethodParamTypes(object, methodName);
    ...
    public static String loadMapJsonString(Map<String, Object> objectMap){
        ...
        for (Map.Entry<String, Object> entry : objectMap.entrySet()) {
            ...
            String valueString;
            LogUtils.i("loadMapJsonString,key=" + key + ",value=" + value);
