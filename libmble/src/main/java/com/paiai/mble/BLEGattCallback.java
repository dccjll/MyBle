...
import com.paiai.mble.util.ByteUtils;
...
public class BLEGattCallback extends BluetoothGattCallback {
    ...
    public void onConnectionStateChange(final BluetoothGatt gatt, int status, int newState) {
        ...
        LogUtils.i(TAG, "onConnectionStateChange,gatt=" + gatt + ",status=" + status + ",newState=" + newState);
        if(status == BluetoothGatt.GATT_SUCCESS){
            if(newState == BluetoothProfile.STATE_CONNECTING){
                LogUtils.i(TAG, "正在连接,gatt=" + gatt + ",status=" + status + ",newState=" + newState);
            }else if(newState == BluetoothProfile.STATE_CONNECTED){
                LogUtils.i(TAG, "已连接,gatt=" + gatt + ",status=" + status + ",newState=" + newState);
                if(onGattBLEConnectListener != null){
                    onGattBLEConnectListener.onConnectSuccss(gatt, status, newState, this);
                }
            }else if(newState == BluetoothProfile.STATE_DISCONNECTING){
                LogUtils.i(TAG, "正在断开,gatt=" + gatt + ",status=" + status + ",newState=" + newState);
            }else if(newState == BluetoothProfile.STATE_DISCONNECTED){
                LogUtils.i(TAG, "已断开,gatt=" + gatt + ",status=" + status + ",newState=" + newState);
                handleError(BLEMsgCode.parseMessageCode(-10007), Log.INFO, gatt);
            }
        }else{
            LogUtils.e(TAG, "收到蓝牙底层协议栈异常消息,gatt=" + gatt + ",status=" + status + ",newState=" + newState);
    ...
    public void onServicesDiscovered(final BluetoothGatt gatt, int status) {
        LogUtils.i(TAG, "onServicesDiscovered,gatt=" + gatt + ",status=" + status);
    ...
    public void onCharacteristicWrite(final BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        LogUtils.i(TAG, "onCharacteristicWrite\ngatt=" + gatt + "\ncharacteristic=" + characteristic + "\nstatus=" + status + "\nwritedData=" + ByteUtils.bytesToHexString(characteristic.getValue())
    ...
    public void onCharacteristicChanged(final BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        ...
        LogUtils.i(TAG, "onCharacteristicChanged\ngatt=" + gatt + "\ncharacteristic=" + characteristic + "\nreceivedData=" + ByteUtils.bytesToHexString(characteristic.getValue())
        ...
        if(bleResponseManager.getBleManage().getRunning() && characteristic.getUuid().toString().equalsIgnoreCase(uuidCharacteristicChange.toString())){//接收到数据
            if(onBLEResponse == null){
                LogUtils.e(TAG, "received data, but onBLEResponse is null");
                return;
            }
    ...
    private synchronized void handleError(String errorMsg, int loglevel, BluetoothGatt gatt) {
        ...
        LogUtils.e(TAG, "handleError,running=" + running);
    ...
    private void onResponseError(String errorMsg, int loglevel, BluetoothGatt gatt){
        if(onGattBLEWriteDataListener != null){
            ...
            LogUtils.e(TAG, "onResponseError, not found listener");
        }
    }
}
