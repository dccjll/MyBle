...
import com.paiai.mble.helper.BLEHelper;
...
public class BLEConnect {
     ...
     */
    public void connect(){
        LogUtils.i(TAG, "准备开始连接");
        LogUtils.i(TAG, "bluetoothDevice=" + bluetoothDevice + "\ntargetMacAddress=" + targetMacAddress);
        ...
        if(bluetoothDevice == null && !MacAddressUtils.checkAddress(targetMacAddress)){
        ...
        if (bluetoothDevices != null && bluetoothDevices.size() > 0) {
            LogUtils.i(TAG, "有连接的设备列表=" + bluetoothDevices);
            ...
            for (BluetoothDevice bluetoothDevice : bluetoothDevices) {
                if (BLEConnect.this.bluetoothDevice == bluetoothDevice) {
                    LogUtils.i(TAG, "根据设备对象匹配到已连接的设备,device=" + bluetoothDevice);
                ...
                if (bluetoothDevice.getAddress().equalsIgnoreCase(bleManage.getTargetDeviceAddress())) {
                    LogUtils.i(TAG, "根据mac地址匹配到已连接的设备,mac=" + bluetoothDevice.getAddress());
        ...
        bleManage.getBleGattCallback().registerOnGattConnectListener(
                new OnGattBLEConnectListener() {
                    ...
                    public void onConnectFail(String errorMsg, int loglevel, BluetoothGatt bluetoothGatt) {
                        LogUtils.i(TAG, "第" + currentConnectCount + "次连接失败\nerrorMsg=" + errorMsg + "\nloglevel=" + LogUtils.getLogTag(loglevel));
                        connect();
                    }
                }
        );
        ...
        LogUtils.i(TAG, "第" + currentConnectCount + "次开始连接");
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                ...
                BluetoothGatt bluetoothGatt = bluetoothDevice.connectGatt(Utils.getApp(), false, bleManage.getBleGattCallback());
                if (bluetoothGatt == null) {
                    LogUtils.e(TAG, "连接失败");
                    connect();
                    return;
                }
                LogUtils.i(TAG, "请求连接成功,gatt=" + bluetoothGatt);
            }
        });
    }
    ...
    private void onConnectDeivceSuccss(BluetoothGatt bluetoothGatt, int status, int newState) {
        if (bluetoothGatt == null) {
            LogUtils.e(TAG, "在连接成功的回调里，bluetoothGatt对象验证失败，bluetoothGatt=null");
            bleManage.handleError(-10051);
            return;
        }
        LogUtils.i(TAG, "已连接成功");
        ...
        if (BLEConfig.START_FIND_SERVICE_INTERVAL > 0) {
            LogUtils.i(TAG, "休眠" + BLEConfig.START_FIND_SERVICE_INTERVAL + "ms开始找服务，手机型号:" + Build.MODEL);
            SystemClock.sleep(BLEConfig.START_FIND_SERVICE_INTERVAL);
        }
        if (!bleManage.getRunning()) {
            LogUtils.e(TAG, "准备找服务时任务已停止");
            return;
        }
