...
public class BLEScan {
    ...
    private final BluetoothAdapter.LeScanCallback leScanCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
            if (!bleManage.getRunning()) {
                LogUtils.i(TAG, "扫描到设备时任务已停止");
                return;
            }
            LogUtils.i(TAG, "扫描到设备,deviceMac=" + device);
            if(foundDevice){
                LogUtils.i(TAG, "已找到设备，略过停止扫描间隙扫描到的设备");
                return;
            }
    ...
    public void startScan(){
        if (!bleManage.getRunning()) {
            LogUtils.i(TAG, "准备开始扫描时任务已停止");
            return;
        }
        LogUtils.i(TAG, "准备开始扫描");
        ...
        if (++currentScanCount > BLEConfig.MAX_SCAN_COUNT) {
            ...
            LogUtils.i(TAG, "扫描失败,已尝试最大扫描次数");
            bleManage.handleError(-10009);
            return;
        }
        if (!BLEConfig.LONG_SCAN_FALG) {//长扫描标记会一直扫描
            scanHandler = new Handler(Utils.getApp().getMainLooper());
            scanRunnable = new Runnable() {
                @Override
                public void run() {
                    scanControl(false);
                    if (!bleManage.getRunning()) {
                        LogUtils.i(TAG, "准备重新扫描时任务已停止");
                        return;
                    }
                    startScan();//重新开始扫描
                }
        ...
        if (!bleManage.getRunning()) {
            LogUtils.i(TAG, "准备扫描时任务已停止");
            return;
        }
        LogUtils.i(TAG, "第" + currentScanCount + "次开始扫描");
        try {
            scanControl(true);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void onFoundDevice(boolean existFilter, BluetoothDevice device, int rssi, byte[] scanRecord) {
        LogUtils.i(TAG, "已找到设备,mac=" + device);
        if (existFilter) {
            foundDevice = true;
            scanControl(false);
