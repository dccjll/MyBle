...
public class BLEOpenNotification {
    ...
    public void openNotification(){
        ...
        LogUtils.i(TAG, "通知的特征属性：" + bluetoothGattCharacteristic.getProperties() + ",need：" + BluetoothGattCharacteristic.PROPERTY_NOTIFY);
    ...
    private void afterOpenNotificationSuccess(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        ...
        LogUtils.i(TAG, "要写入的数据：" + ByteUtils.bytesToHexString(bleManage.getData()));
        if (!bleManage.getRunning()) {
            LogUtils.e(TAG, "准备写入数据时任务已停止");
            return;
        }
