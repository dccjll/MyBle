...
public class BLEWriteData {
    ...
    public void writeData() {
        ...
        dataList = ByteUtils.paeseByteArrayToByteList(data, BLEConfig.MAX_BYTES);
        LogUtils.i(TAG, "要写入的总数据遍历,total data=" + ByteUtils.bytesToHexString(data));
        for(int i=0;dataList != null && i<dataList.size();i++) {
            LogUtils.i(TAG, "i=" + i + ",data=" + ByteUtils.bytesToHexString(dataList.get(i)));
        }
        if (dataList == null) {
            dataWrittenState = false;
        ...
        bleManage.getBleGattCallback().registerOnGattBLEWriteDataListener(
                new OnGattBLEWriteDataListener() {
                    ...
                    public void onWriteDataFinish() {
                        LogUtils.i(TAG, "写入的所有数据为：" + ByteUtils.bytesToHexString(bleManage.getData()));
                    ...
                    public void onWriteDataSuccess(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
                        LogUtils.i(TAG, "数据写入成功，写入的数据为:\t\t\t\t" + ByteUtils.bytesToHexString(characteristic.getValue()) + "\n当前第" + (index + 1) + "包" + "\n总共" + dataList.size() + "包");
                        ...
                        if ((writtenDataLength += characteristic.getValue().length) == data.length) {
                            LogUtils.i(TAG, "数据写入完成");
                            onWriteDataFinish();
                            return;
                        }
                        LogUtils.i(TAG, "间隔" + BLEConfig.SEND_NEXT_PACKAGE_INTERVAL + "ms再发送下一个数据包");
                        SystemClock.sleep(BLEConfig.SEND_NEXT_PACKAGE_INTERVAL);
                        int i = ++index;
                        if(i >= dataList.size()){
                            LogUtils.e(TAG,"写数据时下标越界(出现在底层写成功回调异常情况下，请忽略)");
                            return;
                        }
                    ...
                    public void onWriteDataFail(String errorMsg, int loglevel) {
                        LogUtils.e(TAG, "onWriteDataFail\nerrorMsg=" + errorMsg + "\nloglevel=" + loglevel + "\nbleManage=" + bleManage);
                        dataWrittenState = false;
        ...
        LogUtils.i(TAG, "写数据的特征属性：" + bluetoothGattCharacteristic.getProperties() + ",need：" + BluetoothGattCharacteristic.PROPERTY_WRITE);
