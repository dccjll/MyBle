...
public class ScanDevice {
    ...
    public void startScan() {
        LogUtils.i(TAG, "接收到请求，开始扫描，" + this);
        scanDevice();
    }
    ...
    public void stopScan() {
        LogUtils.i(TAG, "接收到请求，停止扫描，" + this);
        if (bleManage == null) {
            LogUtils.e(TAG, "bleManage == null");
            return;
        }
        bleManage.stopScan();
    }
        ...
        bleManage.setListenterObject(new BLEScan.OnBLEScanListener() {
            ...
            public void onFoundDevice(BluetoothDevice bluetoothDevice, int rssi, byte[] scanRecord) {
                ...
                if (onBLEScanListener != null && !bluetoothDeviceList.contains(bluetoothDevice)) {
                    LogUtils.i(TAG, "发现目标蓝牙设备," + "" + bluetoothDevice.getName() + ",mac=" + bluetoothDevice);
            ...
            public void onScanFail(int errorCode) {
                LogUtils.i(TAG, BLEMsgCode.parseMessageCode(errorCode));
